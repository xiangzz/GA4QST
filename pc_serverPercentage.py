...
str = filename + '/' + 'serverPercentage' + time_str
csv_name = str + '.csv'
...
if __name__ == '__main__':
    ...
    max_server_num = server_num + switch_num

    deviceList = generateDevice(ap_num, max_server_num, 0, avg_lamda, bias_lamda, avg_v, bias_v, avg_c, bias_c,
                                serviceList)
    for i in range(0, 5):
        # 参数变化过程(1~500 , 100间隔)
        deviceList.remove(deviceList[ap_num + max_server_num-1-i])
        deviceList.insert(ap_num + max_server_num-1-i, Switch(ap_num + max_server_num-1-i))
        ...
        # plot值
        generation = []
        g_best_rho = [(max_server_num - i - 1)/max_server_num]
        Cls = []
        Lls = []
        Tls = []
