...
str = filename + '/' + 'ZDensity' + time_str
csv_name = str + '.csv'
...
if __name__ == '__main__':
    ...
    for i in range(0, 5):
        ...
        count = 0
        noPathCount = 0
        for j in range(AQ_NUM, len(Z)):
            for k in range(AQ_NUM, len(Z[j])):
                if Z[j][k] <= 0:
                    if count >= 1:
                        noPathCount = noPathCount + 1
                        continue
                    else:
                        Z[j][k] = random.uniform(avg_ZTransRate - bia_ZTransRate, avg_ZTransRate + bia_ZTransRate)
                        Z[k][j] = Z[j][k]
                        count = count + 1
        count_n = len(Z - AQ_NUM)

        identifier = (count_n*(count_n-1)/2 - noPathCount) / (count_n*(count_n-1)/2)
