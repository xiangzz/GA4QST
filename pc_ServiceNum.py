...
str = filename + '/' + 'serviceNum' + time_str
csv_name = str + '.csv'
...
if __name__ == '__main__':
    ...
    for i in range(0, 5):
        # 参数变化过程
        new_service = generateServices(1, avg_rqSize, bias_rqSize, avg_exeRate, bias_exeRate, avg_price, bias_price)[0]
        new_service.serviceId = SERVICE_NUM
        serviceList.append(new_service)
        ...
        for ap in deviceList:
            ...
            ap.arrivalRate.append(lamda)
        for s in deviceList:
            if(isinstance(s, Server) == 0):
                continue
