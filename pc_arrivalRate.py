...
str = filename + '/' + 'arrivalRate' + time_str
csv_name = str + '.csv'
    ...
    for i in range(0, 5):
        ...
        for d in deviceList:
            if isinstance(d, AccessPoint):
                for ar in range(0, len(serviceList)):
                    d.arrivalRate[ar] = 4+4*i
                    identifier = d.arrivalRate[ar]
        # 实验过程
        POP = []
        # 生成种群
        ...
        for n_gen in range(0, N_GEN):
            ...
            # 5.26新增
            min_max_row = evaluatePopCTL(POP, allLegalPaths, serviceList, deviceList, Z)
